...
def register():
    ...
    "✅ पंजीकरण सफल रहा। कृपया लॉगिन करें।"
))
    return redirect(url_for('account'))
...
def login():
    ...
    if not valid:
        ...
        # ✅ LOGIN SUCCESS (LANGUAGE SAFE)

    # 🔹 pehle language save karo
    lang = session.get('lang', 'en')

    # 🔹 ab session clear karo
    session.clear()

    # 🔹 language wapas daalo
    session['lang'] = lang
...
def category_products(cat_name):
    ...
    categories_map = {
        ...
        'vegetables': {"en": "Fresh Vegetables", "hi": "ताज़ी सब्ज़ियाँ"},
        'fruits': {"en": "Fresh Fruits", "hi": "ताज़े फल"},
        'spices-herbs': {"en": "Spices & Herbs", "hi": "मसाले और जड़ी-बूटियाँ"},
        'oil-seeds': {"en": "Oil Seeds & Nuts", "hi": "तेलहन और मेवे"},
        'other': {"en": "Other", "hi": "अन्य"}
    }

    # 1️⃣ URL se category nikalo
    cat = categories_map.get(cat_name)

    # 2️⃣ Agar galat URL ho
    if not cat:
        flash(t(
            "❌ Category not found.",
            "❌ श्रेणी नहीं मिली।"
    ...
    # 3️⃣ Database ke liye English category
    db_category = cat["en"]

    # 4️⃣ Products fetch karo
    cur = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    ...
    # 5️⃣ Page heading language ke hisaab se
    page_category = t(cat["en"], cat["hi"])

    # 6️⃣ Template ko bhejo
    return render_template(
...
def add_to_cart(product_id):
    ...
    "❌ आप अपना ही उत्पाद नहीं खरीद सकते।"
), "danger")
        return redirect(request.referrer or url_for('products_page'))
    ...
    "✅ उत्पाद कार्ट में जोड़ दिया गया है!"
), "success")
    return redirect(request.referrer or url_for('products_page'))
...
def cart():
    ...
    "❌ कार्ट देखने के लिए कृपया लॉगिन करें।"
), "danger")
        return redirect(url_for('account'))
...
def add_to_wishlist(product_id):
    ...
    "❗ यह पहले से आपकी विशलिस्ट में है।"
), "info")
...
def add_product():
    ...
    if request.method == 'POST':
        ...
        # 🔴 IMPORTANT: Hindi description only
        description = request.form.get('description', '').strip()

        if not description:
            flash("❌ कृपया उत्पाद का विवरण (Description) हिन्दी में भरें", "danger")
        ...
        # ===== INSERT PRODUCT (Hindi description) =====
        cur = mysql.connection.cursor()
        cur.execute("""
            INSERT INTO products
            (farmer_id, name, category, variety, price_min, price_max, image,
             stock_status, description, weights, quantity, contact_number,
             negotiable, location, delivery_available, harvest_date, district)
        ...
        """, (
            ...
            stock_status,
            description,              # ✅ Hindi saved directly
            json.dumps(weights),
            quantity,
            contact_number,
            negotiable,
        ...
        flash("✅ उत्पाद सफलतापूर्वक जोड़ दिया गया", "success")
        return redirect(url_for('farmer_dashboard'))
